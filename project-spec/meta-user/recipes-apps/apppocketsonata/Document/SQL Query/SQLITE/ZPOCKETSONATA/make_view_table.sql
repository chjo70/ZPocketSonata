...
*/			
		...
		SELECT 
			...
			RADARMODE.FIRST_TECH_NO,
			RADARMODE.SECOND_TECH_NO
...
CREATE VIEW VEL_ACTIVE_AET_LIST AS
	...
	FROM AETDATA AS t1, ( SELECT AETID, MAX(rowid) AS max_aetid, MAX(OP_INIT_ID) AS MAX_OP_INIT_ID FROM AETDATA GROUP BY AETID ) AS t2
	WHERE t1.ROWID = t2.MAX_AETID AND t1.AETID = t2.AETID AND t1.STAT != 4 AND t1.OP_INIT_ID = t2.MAX_OP_INIT_ID
...
CREATE VIEW VEL_ACTIVE_ABT_LIST AS
	...
	FROM ABTDATA AS t1, ( SELECT AETID, ABTID, MAX(rowid) AS MAX_AETID, MAX(OP_INIT_ID) AS MAX_OP_INIT_ID FROM ABTDATA GROUP BY AETID, ABTID ) AS t2
...
CREATE VIEW VEL_ACTIVE_PEVALID_AET_LIST AS
	...
	FROM AETDATA AS t1, ( SELECT AETID, ABTID, max(rowid) AS max_aetid FROM ABTDATA GROUP BY AETID, ABTID ) AS t2
...
CREATE VIEW VEL_ACTIVE_PEVALID_ABT_LIST AS 
	...
	FROM ABTDATA AS t1, ( SELECT AETID, ABTID, max(rowid) AS MAX_AETID, MAX(OP_INIT_ID) AS OP_INIT_ID FROM ABTDATA GROUP BY AETID, ABTID ) AS t2
	WHERE t1.ROWID = t2.MAX_AETID AND t1.AETID = t2.AETID AND t1.ABTID = t2.ABTID AND t1.STAT != 4 AND t1.PE_VALID == 1
	ORDER BY t1.LAST_TIME DESC;
