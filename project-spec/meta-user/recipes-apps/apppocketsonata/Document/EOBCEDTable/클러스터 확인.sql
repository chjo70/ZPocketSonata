...
select avg_row_len from user_tables where table_name='E_AA_MSG_LOB';

select avg(count(*)) from E_AA_MSG_LOB group by MISSION_ID;

create cluster E_AA_MSG_LOB# (MISSION_ID char(22));

rename E_AA_MSG_LOB to E_AA_MSG_LOB_TMP;
...
) cluster E_AA_MSG_LOB#(MISSION_ID);