...
private:
    ...
    STR_TRKSCNPDWDATA *m_pScnPDWData;                                                   ///< 메시지 데이터 포인터
    SRxABTData *m_pstABTData;                                                           ///< 메시지 데이터 포인터

public:
    STR_MessageData *m_pMsg;															///< CThread 에서 사용하는 단일 메시지 데이터 포인터 입니다.
    SRxLOBData m_stLOBData;                                                             ///< 스캔 분석 결과 저장소

private:
    void InitOfMessageData();                                                            ///< 메시지 처리시 초기 설정

    void AnalysisStart();																///< 입력 PDW 데이터로 스캔 분석을 수행합니다.
    void MakeLOBData();                                                                 ///< LOB에 스캔 분석 결과를 저장합니다.

    void InitScanAnalysis();                                                            ///< 스캔 분석 관련 메모리 초기화

public:
    CScanAnalysis( int iThreadPriority, const char *pThreadName=NULL, bool bArrayLanData=false );		///< 초기 멤버 변수값 등을 설정하는 객체 생성자 입니다.
