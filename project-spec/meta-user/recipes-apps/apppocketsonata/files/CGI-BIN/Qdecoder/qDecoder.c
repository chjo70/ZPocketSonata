...
#include <io.h>
#include <direct.h>
//extern unsigned standard_input;
...
static void _bit_progressbar(char *upload_id);
static int _upload_getstatus(char *upload_id, long *upload_tsize, long *upload_csize, char *upload_cname);
...
#ifdef VXWORKS
char upload_savedir[1024]={"PRGDISK:"};
#elif defined(__linux__)
char upload_savedir[1024]={"/home/chjo70" };

#else
static char upload_savedir[1024]={"c:\\upload" };
#endif
...
int qDecoder(void) {
    ...
    if(content_type == NULL) {
        ...
        if(_first_entry != NULL) {
            ...
            q_bit_id = qValue("Q_BIT_ID");

            if(q_upload_id != NULL) {
                if(_upload_base_init == 0) {
                    qError("qDecoder(): qDecoderSetUploadBase() must be called before.");
                }
                _upload_progressbar(q_upload_id);
            }
            else if( q_bit_id != NULL) {
                _bit_progressbar( q_bit_id );
...
static int _parse_multipart_data(void) {
...
#ifdef _WIN32
		//ReadFile( standard_input, buf, sizeof(buf), & count );
		//for(j = 1; (count=_read( standard_input, buf, sizeof(buf))) > 0 ; j++) {
		for(j = 1; (count=_fgets(buf, sizeof(buf), stdin)) != 0 ; j++) {
#else
    for(j = 1; (count=_fgets(buf, sizeof(buf), stdin)) != 0 ; j++) {
#endif
      printf("Line %d, len %d : %s", j, count, buf);
			for(i=0; i < count ; i++) printf("%1c",buf[i]);
			printf("<br>\n");
  ...
  /* check boundary */
#ifdef _WIN32
  if(_fgets(buf, sizeof(buf), stdin) == 0) qError("_parse_multipart_data(): Your browser sent a non-HTTP compliant message.");
	//if(_read( standard_input, buf, sizeof(buf)) == 0) qError("_parse_multipart_data(): Your browser sent a non-HTTP compliant message.");
#else
  if(_fgets(buf, sizeof(buf), stdin) == 0) qError("_parse_multipart_data(): Your browser sent a non-HTTP compliant message.");
#endif
  ...
  for(finish = 0, _post_cnt = 0; finish != 1; amount++, _post_cnt++) {
    ...
    //qErrorPrint( "_parse_multipart_data_finish[%d], _first_entry[%d], upload_type[%d]" ,  finish, _first_entry, upload_type );
    ...
    if(_first_entry != NULL && upload_type == 0) {
      ...
      if(upload_id != NULL) {
        ...
        qErrorPrint( "CONTENT_LENGTH=%ld" , atol(getenv("CONTENT_LENGTH")) );
        qErrorPrint( "QUERY_STRING=%s" , getenv("QUERY_STRING") );
        if(qCountSave(upload_tmppath, atol(getenv("CONTENT_LENGTH"))) == 0) qError("_parse_multipart_data(): Can not save uploading information at %s", upload_tmppath);
...
static char *_upload_getsavedir(char *upload_id, char *upload_savedir) {
  ...
  //printf( "\n upload_savedir[%s]", upload_savedir );

  return upload_savedir;
}

/**
 * @brief _bit_progressbar
 * @param bit_id
 */
static void _bit_progressbar(char *bit_id) {
    long upload_tsize = 0, upload_csize = 0;
    char upload_cname[256];

    int  drawrate = qiValue("Q_BIT_DRAWRATE");
    char *template = qValue("Q_BIT_TEMPLATE");

    /* print out qDecoder logo */
    qContentType("text/html");

    qErrorPrint( "_bit_progressbar bit_id[%s]" , bit_id );

    if(template == NULL) {
      printf("_print_progressbar(): Q_ID_TEMPLATE query not found.");
      return;
    }
    ...
    int iCount = 5;
    while( iCount > 0 ) {
        qErrorPrint( "iCount[%d]" , iCount );

        printf("<script language='JavaScript'>");
        printf("qSetProgress(%ld,%ld,'%s');", upload_tsize, upload_csize, upload_cname);
        printf("</script>\n");
        ...
        // Sleep( drawrate );
#else
        //qErrorPrint( "drawrate[%d]" , drawrate );
...
static void _upload_progressbar(char *upload_id) {
  ...
  qErrorPrint( "_upload_progressbar upload_id[%s]" , upload_id );
  ...
  qErrorPrint( "template[%s]" , template );
  ...
  while(1) {
    ...
    if(last_csize < upload_csize) {
      ...
      qErrorPrint("qSetProgress(%ld,%ld,%s)", upload_tsize, upload_csize, upload_cname );
      ...
      if(freezetime > 10000) {
        qErrorPrint("qSetProgress(%ld,%ld,%s)...freezetime", upload_tsize, upload_csize, upload_cname );
...
static int _upload_getstatus(char *upload_id, long *upload_tsize, long *upload_csize, char *upload_cname) {
  ...
  *upload_tsize = *upload_csize = 0;
  ...
  /* open upload folder */
	sprintf( buffer, ".\\%s\\*.*" , upload_savedir );
  ...
  *upload_tsize = qCountRead( upload_filepath );
    ...
    if( c_file.name[0]-'0' <= 0 || c_file.name[0]-'0' > 9) continue; /* first char must be a number */

    /* sort last filename */
    if(strcmp(upload_cname, c_file.name) < 0) strcpy(upload_cname, c_file.name);

    sprintf(upload_filepath, "%s/%s", upload_savedir, c_file.name);
  ...
  *upload_tsize = *upload_csize = 0;
  ...
  /* get basepath */
  if(_upload_getsavedir(upload_id, upload_savedir) == NULL) {
      qErrorPrint( "_upload_getstatus(): Q_UPLOAD_ID does not set.");
      qError("_upload_getstatus(): Q_UPLOAD_ID does not set.");
  }

  /* open upload folder */
  if((dp = opendir(upload_savedir)) == NULL) { qErrorPrint( "upload_savedir[%s]" , upload_savedir ); return 0; }

  /* read tsize */
  sprintf(upload_filepath, "%s/Q_UPLOAD_TSIZE", upload_savedir);
  *upload_tsize = qCountRead(upload_filepath);
  qErrorPrint( "upload_filepath[%s], upload_tsize[%ld]" , upload_filepath, *upload_tsize );
...
int qiValue(const char *format, ...) {
  ...
  //qErrorPrint( "qiValue[%s]", name );
  if(_first_entry == NULL) qDecoder();
  return _EntryiValue(_first_entry, name);
}

long qlValue(const char *format, ...) {
  char name[1024];
  int status;
  va_list arglist;
  ...
  va_start(arglist, format);
  status = vsprintf(name, format, arglist);
  if(strlen(name) + 1 > sizeof(name) || status == EOF) qError("qlValue(): Message is too long or invalid.");
  va_end(arglist);

  //qErrorPrint( "qiValue[%s]", name );
  if(_first_entry == NULL) qDecoder();
  return _EntrylValue(_first_entry, name);
}
