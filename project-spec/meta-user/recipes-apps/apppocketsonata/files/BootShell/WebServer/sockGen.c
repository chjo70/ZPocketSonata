...
{
#if (!defined (NO_GETHOSTBYNAME) && !defined (__VXWORKS__))
	struct hostent		*hostent;					/* Host database entry */
#endif /* ! (NO_GETHOSTBYNAME || __VXWORKS__) */
	socket_t			*sp;
 ...
 */
			...
			if (sockaddr.sin_addr.s_addr == (unsigned long) NULL) {
				errno = ENXIO;
				socketFree(sid);
				return -1;
			}
