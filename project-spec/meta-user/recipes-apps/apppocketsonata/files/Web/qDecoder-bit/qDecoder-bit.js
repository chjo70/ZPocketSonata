...
var Q_UPLOAD_TEMPLATE="../qDecoder-bit/qDecoder-bit.html";	// set progress template
...
function qBit(qForm) {
	...
	if (qBrowserVersion.indexOf('MSIE') != -1 && qBrowserVersion.substr(qBrowserVersion.indexOf('MSIE')+5,1) > 4) {
		...
		window.open(q_upload_action+"?Q_UPLOAD_ID="+q_upload_id+"&Q_UPLOAD_DRAWRATE="+Q_UPLOAD_DRAWRATE+"&Q_UPLOAD_TEMPLATE="+Q_UPLOAD_TEMPLATE, null, "width=680,height=710,status=no,toolbar=no,menubar=no,location=no,resizable=no,scrollbars=no,copyhistory=no");
	}	
