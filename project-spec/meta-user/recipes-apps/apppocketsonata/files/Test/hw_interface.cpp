...
//	ui32 reg;
	...
	while(1)
	{
		...
		case UIO_DMA_1_ADDR :
		{
			...
			for(int k = 0; k <PDW_GATHER_SIZE/32; k++)
			{
                int chno;
                float rFreq;

                for( j=0 ; j < 24 ; ++j ) {
                    printf( "%02X ", pbuf[j+(32*k)] );
                    if( j % 16 == 0 && j !=0 ) {
                        //printf( "\n"  );
                    }
                }
                ...
                sToa	= ( (unsigned long long int) s_pdw_reg_t.uniPdw_toa_edge.stPdw_toa_edge.toa_H << 16) | s_pdw_reg_t.uniPdw_freq_toa.stPdw_freq_toa.toa_L;

                printf("s_pdw_reg_t = %ld\n ",sizeof(s_pdw_reg_t));
                ...
                printf(">> fmop_bw 	= %f(0x%X)\n", s_pdw_reg_t.uniPdw_status.stPdw_status.fmop_bw*PDW_FREQ_RES, s_pdw_reg_t.uniPdw_status.stPdw_status.fmop_bw );
				printf(">> doa 		= %f\n", s_pdw_reg_t.uniPdw_dir_pa.stPdw_dir_pa.doa*PDW_AOA_RES); //deg
				printf(">> di	 		= %d\n", s_pdw_reg_t.uniPdw_dir_pa.stPdw_dir_pa.di);
				printf(">> pa 		= %d\n", s_pdw_reg_t.uniPdw_dir_pa.stPdw_dir_pa.pa);
                printf(">> pw			= %f[ns](0x%X)\n", s_pdw_reg_t.uniPdw_pw_freq.stPdw_pw_freq.pulse_width*PDW_TIME_RES, s_pdw_reg_t.uniPdw_pw_freq.stPdw_pw_freq.pulse_width ); //ns
                printf(">> frequency	= %.3f[MHz](0x%X)\n", rFreq, sFreq );
