...
void CKPulExt::KnownPulseExtract()
{
    ...
    switch( m_pTrkAet->iPRIType ) {
		...
		case _STAGGER :
			...
			*/
            ExtractTrackPT( m_pTrkAet->fMeanPRI, FDIV( m_pTrkAet->fPRIJitterRatio, 100 ) );
			break;

		case _DWELL :
            for( i=0 ; i < m_pTrkAet->iPRIPositionCount ; ++i ) {
                extRange.min_pri = m_pTrkAet->fPRISeq[i] - ( 2 * STABLE_MARGIN );
                extRange.max_pri = m_pTrkAet->fPRISeq[i] + ( 2 * STABLE_MARGIN );
		...
		case _JITTER_RANDOM :
			...
			*/
			// 지터열 추출 마진 설정
            diff = UDIV( m_pTrkAet->fMeanPRI * ( m_pTrkAet->fPRIJitterRatio+EXTRACT_JITTER_MARGIN ), 200 );
            extRange.min_pri = _max( (int)(m_pTrkAet->fMeanPRI - diff), 2 );
            extRange.max_pri = m_pTrkAet->fMeanPRI + diff;
			ExtractStablePT( & extRange, TRUE );
			DiscardStablePT();
			...
			*/
            diff = UDIV( m_pTrkAet->fMeanPRI * ( m_pTrkAet->fPRIJitterRatio+20 ), 200 );
            extRange.min_pri = _max( (int)(m_pTrkAet->fMeanPRI - diff), 2 );
            extRange.max_pri = m_pTrkAet->fMeanPRI + diff;
			// 추출할 펄스열의 범위폭을 계산한다.
			ExtractJitterPT( & extRange, -1, 3, TRUE );
		...
		case _JITTER_PATTERN :
            ...
            extRange.min_pri = m_pTrkAet->fMinPRI - diff;
            extRange.max_pri = m_pTrkAet->fMaxPRI + diff;
