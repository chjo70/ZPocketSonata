...
void CKMakeAET::MakeUpAET()
{
	...
	if( pUpdAet != NULL ) {
        ...
        if( CheckHarmonic( m_pTrkAet, pUpdAet ) >= 2 ) {
            ...
            pUpdAet->fMeanPRI = m_pTrkAet->fMeanPRI;
            pUpdAet->fMaxPRI = m_pTrkAet->fMaxPRI;
            pUpdAet->fMinPRI = m_pTrkAet->fMinPRI;
	...
	// 추적 에미터가 발견되지 않을 때는 새로운 에미터 제원으로 추적 업데이트한다.
		...
		for( int i=0 ; i < count ; ++i ) {
            if( CheckHarmonic<SRxABTData>( m_pTrkAet, pNewAet ) == TRUE ) {
                ...
                pNewAet->fMeanPRI = m_pTrkAet->fMeanPRI;
                pNewAet->fMaxPRI = m_pTrkAet->fMaxPRI;
                pNewAet->fMinPRI = m_pTrkAet->fMinPRI;
    ...
    if( m_pTrkAet->iFreqType != pUpdAetFrq->iFreqType ) {
        switch( m_pTrkAet->iFreqType ) {
            case _HOPPING :
                ...
                pUpdAetFrq->fMeanFreq = m_pTrkAet->fMeanFreq;
                pUpdAetFrq->fMaxFreq = m_pTrkAet->fMaxFreq;;
                pUpdAetFrq->fMinFreq = m_pTrkAet->fMinFreq;;
