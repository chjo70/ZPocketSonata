...
void CKPulExt::KnownPulseExtract()
{
	...
	switch( pPri->type ) {
		case _STABLE :
			extRange.min_pri = pPri->min - STABLE_MARGIN;
			extRange.max_pri = pPri->max + STABLE_MARGIN;
		...
		case _STAGGER :
			...
			*/
			ExtractTrackPT( stTrkAet.aet.pri.mean, FDIV( stTrkAet.aet.pri.jtrPer,100 ) );
			break;

		case _DWELL :
			for( i=0 ; i < pPri->swtLev ; ++i ) {
				extRange.min_pri = pPri->swtVal[i] - ( 2 * STABLE_MARGIN );
				extRange.max_pri = pPri->swtVal[i] + ( 2 * STABLE_MARGIN );
				ExtractDwellRefPT( & extRange );
			}
			ExtractRefStable();
			break;
		...
		case _JITTER :
			...
			*/
			// 지터열 추출 마진 설정
			diff = UDIV( pPri->mean * ( pPri->jtrPer+EXTRACT_JITTER_MARGIN ), 200 );
			extRange.min_pri = _max( (int)(pPri->mean - diff), 2 );
			extRange.max_pri = pPri->mean + diff;
		...
		case _PATTERN :
			...
			extRange.min_pri = pPri->min - diff;
			extRange.max_pri = pPri->max + diff;
...
//! \warning
//
void CKPulExt::CalPRIRange( STR_PULSE_TRAIN_SEG *pSeg, UINT priMean, UINT dtoa_count )
{
...
//! \warning
//
void CKPulExt::MakeDtoaHistogram( PDWINDEX *pPdwIndex, int count, STR_MINMAX *pRange )
{
