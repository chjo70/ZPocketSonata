 ...
 * @warning
 */
bool CELEmitterMergeMngr::ManageThreat( SRxLOBHeader* pLOBHeader, SRxLOBData* pLOBData, SLOBOtherInfo *pLOBOtherInfo, bool i_bIsFilteredLOB, bool i_bCheckLOB )
{
    bool bRtn = true;
    bool bMerge = false;
...
void CELEmitterMergeMngr::UpdatePEInfo( SRxAETData *pAETData, SELAETDATA_EXT *pAETExtData, CELThreat *pTheAETThreat, SRxABTData *pManABTData )
{
    ...
    if( pAETData->uiCoABT == _spOne ) {
        ...
        if( pABTData != NULL ) {
            ...
            pAETData->dLatitude = pABTData->dLongitude;
            pAETData->dLatitude = pABTData->dLatitude;
        ...
        */
        if( fmaxMajor != _spZero ) {
            if( pTheMajorABTThreat != NULL ) {
                ...
                pAETData->dLongitude = pABTData->dLongitude;
                pAETData->dLatitude = pABTData->dLatitude;
                //pAETData->peInfo.iAltidude = pABTData->peInfo.iAltidude;
                pAETData->fCEP = pABTData->fCEP;
                pAETData->fMajorAxis = pABTData->fMajorAxis;
...
void CELEmitterMergeMngr::CreateAETThreat( CELThreat *pAETThreat, CELThreat *pABTThreat, SRxLOBData *pLOBData, SELLOBDATA_EXT *pLOCDataExt )
{
    ...
    // CED/EOB 식별 정보
    pAETData->iRadarModeIndex = 0;
    pAETData->iThreatIndex = 0;

    // 위치 산출 정보
    pAETData->iPEValid = 0;
    pAETData->dLatitude = 0;							// [deg]
    pAETData->dLongitude = 0;							// [deg]
    pAETData->fCEP = 0;										// [m]
    pAETData->fMajorAxis = 0;							// [m]
    pAETData->fMinorAxis = 0;							// [m]
    pAETData->fTheta = 0;									// [0.1도]
    pAETData->fDistanceErrorOfThreat = 0;	// [m]
...
{
    UINT i;
    std::vector<SRxAETData>::pointer pVecAETData;

    pVecAETData = m_VecAETData.data();
    for( i=0 ; i < m_VecAETData.size() ; ++i ) {
        if( pVecAETData->uiAETID == pAETData->uiAETID ) {
            m_VecAETData.erase( m_VecAETData.begin()+i );
            break;
        }
 ...
 * @param bUpdateThreat
 */
void CELEmitterMergeMngr::InsertToDB_AET( SRxAETData *pAETData, SELAETDATA_EXT *pAETExtData, SELEXTDB *pExtDB )
{
#ifdef _SQLITE_
    struct tm stTime;
    char buffer1[100], buffer2[100], buffer3[100];

    _localtime32_s(& stTime, & pAETData->tiFirstSeenTime );
    strftime( buffer1, 100, "%Y-%m-%d %H:%M:%S", &stTime);
    _localtime32_s(&stTime, & pAETData->tiLastSeenTime );
    strftime( buffer2, 100, "%Y-%m-%d %H:%M:%S", &stTime);
    ...
    sprintf_s( m_pszSQLString, "INSERT INTO AETDATA (AETID, RADARMODE_INDEX, THREAT_INDEX, IDINFO, ELNOT_PRIM, MODE_CODE_PRIM, NICK_NAME, PRIM_FUNC_CODE, RADAR_PRIORITY, PIN_NUM, SITE_NAME, BE_NUM, THREAT_FUNC_CODE, THREAT_PRIORITY, DISTANCE_ERR_FROM_THREAT, EQUIP_NUM, NUM_OF_LOBS, NUM_OF_BEAMS, RECORD_TIME_STA, RECORD_TIME_FIN, VALIDITY, FRQ_MEAN, FRQ_MAX, FRQ_MIN, FRQ_DEV, PRI_MEAN, PRI_MAX, PRI_MIN, PRI_DEV, PW_MEAN, PW_MAX, PW_MIN, PW_DEV, PA_MEAN, PA_MAX, PA_MIN, PA_DEV, POS_EST_VALID, POS_EST_LAT, POS_EST_LONG, POS_EST_HEIGHT, POS_EST_CEP, POS_EST_MAJOR_AXIS, POS_EST_MINOR_AXIS, EEP_MAJOR_TILT_ANGLE, FINAL_ALARM_TIME, IS_MANUAL_INPUT, MANUALPOSESTPREFERRED, MANUAL_POS_EST_LAT, MANUAL_POS_EST_LONG, STAT ) VALUES \
                                ...
                                pAETData->fMeanFreq, pAETData->fMaxFreq, pAETData->fMinFreq, pAETData->fDeviationFreq, pAETData->fMeanPRI, pAETData->fMaxPRI, pAETData->fMinPRI, pAETData->fDeviationPRI, \
                                pAETData->fMeanPW, pAETData->fMaxPW, pAETData->fMinPW, pAETData->fDeviationPW, pAETData->fMeanPA, pAETData->fMaxPA, pAETData->fMinPA, pAETData->fDeviationPA, \
                                pAETData->iPEValid, pAETData->dLatitude, pAETData->dLongitude, pAETData->fAltidude, pAETData->fCEP, pAETData->fMajorAxis, pAETData->fMinorAxis, pAETData->fTheta, \
                                buffer3, pAETExtData->xMannualEdited.val32, pAETExtData->bManualPosEstPreferred, pAETData->iManualLatitude, pAETData->iManualLongitude, pAETExtData->enBeamEmitterStat );
