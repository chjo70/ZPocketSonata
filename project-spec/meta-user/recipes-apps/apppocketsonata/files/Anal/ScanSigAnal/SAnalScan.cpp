...
//
CSAnalScan::CSAnalScan( void *pParent, int coMaxPdw ) : CAnalPRI( coMaxPdw )
{
...
EN_SCANRESULT CSAnalScan::AnalScan( int preAnalStat )
{
    ...
    */
    ...
    paMean = (UINT) ( MeanInArray( & m_pScanPt->pa[0], m_pScanPt->co ) + 0.5 );

    if( CheckSteadySignal( & m_nSample, paMean ) == _spTrue ) {
        m_nScnTyp = STEADY;
        m_nScnPrd = _spZero;
...
void CSAnalScan::CalcSamplingTime()
{
	...
	// Cal. sampling time
    priMean = m_pScnAet->fPRIMean; // stScnAet[ m_noEMT ].aet.pri.max;
	...
	*/
	m_nSampleTime = CalcSamplingTime2( m_noEMT, priMean );
...
UINT CSAnalScan::CalcSamplingTime( UINT priMean )
{
	...
	// Calculate Sampling Time (1)
	sampleTimeByPulse = UDIV( spanTime, _spMaxSample );

	sampleTimeByPri = 5 * priMean;		// debug, 00-07-27 12:09:07

	if( sampleTimeByPulse < sampleTimeByPri )
		return sampleTimeByPri;
	else
		return sampleTimeByPulse;
		...
		if( *px < dShgh ) {
			...
			*psx = dShgh - m_nSampleTime;
			if( inpulse != _spZero ) {
				...
				*psy = (UINT) ( _spZero - 1 );					// marking none sampling pulse
			}
			dShgh += m_nSampleTime;
...
//
UINT CSAnalScan::CheckSteadySignal( STR_SAMPLE *pSample, UINT meanY )
{
	UINT i;
	UINT cleanPa;

	UINT *pPa;
    UINT thPa=100; //UDBCNV( 3.0, 1 );			// 3dBm 이하 허용하게 함.
	...
	if( ( FDIV( cleanPa, pSample->co ) < 0.01 ) && ( 1 || pSample->sdevY < UDIV( 1.0, _spAMPres ) ) ||
		cleanPa <= 2 ) {
...
UINT CSAnalScan::ScanTypeDecision2( STR_SAMPLE *pSample, STR_AUTOCOR *pAcf )
{
	...
	pAcf->idPeak = prdVer = PeriodVerify();

  // TurnOffLED( prc_SAP, LED1 );        // debug, 00-06-05 09:39:34
...
void CSAnalScan::KurtosisSkewness( STR_SAMPLE *pSample )
{
  ...
  double A, B, B3, B4, C, N;
	...
	pSample->pa[0] = pSample->pa[2] = pSample->pa[4] = pSample->pa[6] = pSample->pa[8] = 10;
	pSample->pa[1] = pSample->pa[3] = pSample->pa[5] = pSample->pa[7] = pSample->pa[9] = 13;
	pSample->pa[10] = pSample->pa[12] = pSample->pa[14] = pSample->pa[16] = pSample->pa[18] = 10;
	pSample->pa[11] = pSample->pa[13] = pSample->pa[15] = pSample->pa[17] = pSample->pa[19] = 10;
	pSample->pa[5] = 10;
	for( i=0 ; i < pSample->co ; ++i ) {
		// pSample->pa[i] = i;
		sum += pSample->pa[i];
	...
	*/
    ...
    pSample->kurtosis = pSample->skewness = _spZero;
		co = -1;
	}
	else {
    N = (double) pSample->co;
...
UINT CSAnalScan::PeriodVerify( void )
{
	...
	virPeak = prdPeak = UDIV( m_nScnPrd, m_nSampleTime );
	...
	for( i=0 ; i < m_nCoCanPeak ; ++i ) {
		...
		if( virPeak < m_nCanPeak[m_nCoCanPeak-1]+maxErr ) {
			if( CompMeanDiff( virPeak, m_nCanPeak[i], maxErr ) == _spTrue ) {
