...
static TCHAR *ini_strncpy(TCHAR *dest, const TCHAR *source, size_t maxlen, enum quote_option option)
{
  ...
  case QUOTE_DEQUOTE:
    s = 0;
    for (d = 0; source[s] != '\0' && d < maxlen - 1; s++, d++) {
                        ...
                        INI_FILEPOS *mark)
{
  ...
  do {
    ...
    if (ep == NULL)
      ep = _tcschr(sp, ':');
  } while (*sp == ';' || *sp == '#' || ep == NULL || ((len == 0 || (int)(skiptrailing(ep,sp)-sp) != len || _tcsnicmp(sp,Key,len) != 0) && ++idx != idxKey));
