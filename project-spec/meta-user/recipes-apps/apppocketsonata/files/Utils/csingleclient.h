...
public:
    CSingleClient( int iKeyId, const char *pClassName, unsigned short iPort, char *pServerAddress=NULL, void *pParent=NULL );	///< 초기 멤버 변수값등을 설정하는 객체 생성자 입니다.
	...
	int SendLan(UINT uiOpCode, void *pData, UINT uiLength, BOOL bSwap=TRUE );								///< 연결된 서버에 데이터를 송신한다.
