 ...
 * Init functions
******************************************************************/
//void dma_init( dma_dev_t dev, mem_dev_t mem )
//{
//	dma[dev].reg = ((volatile void *)(mem_get_mem(mem)->logical));
//}
...
{
	if ( length > (list->uiSize * list->uiCount) ) return -1;
...
bool dma_s2mm_start( dma_dev_t dev, xmem_t *dst, uint32_t length, bool wait_done )
{
    ...
    if( dma[dev].pReg != NULL ) {
        ...
        LOGMSG( enError, "Error of dma_s2mm_start() !!!" );
        ret = false;
    }
...
void *dma_mm2s_get_mem( dma_dev_t dev, uint32_t *size )
{
	if (dma[dev].mm2s_desc.curr->status.cmplt == 1)
	{
		*size = dma[dev].mm2s_desc.curr->status.transferred;
        return (void *)mem_p2l( dma[dev].pmm2s_mem, dma[dev].mm2s_desc.curr->buffer_addr );
	}
	else
	{
		*size = 0;
...
void dma_mm2s_reset(dma_dev_t dev)
{
    if( dma[dev].pReg != NULL ) {
        ...
        LOGMSG( enError, "Error of dma_mm2s_reset() !!!" );
    }
}

/**
 * @brief dma_s2mm_reset
 * @param dev
 */
void dma_s2mm_reset(dma_dev_t dev)
{
    if( dma[dev].pReg != NULL ) {
        ...
        LOGMSG( enError, "Error of dma_s2mm_reset() !!!" );
    }
}
...
void dma_mm2s_stop(dma_dev_t dev)
{
    if( dma[dev].pReg != NULL ) {
        ...
        LOGMSG( enError, "Error of dma_mm2s_stop() !!!" );
    }
}
...
void dma_s2mm_stop(dma_dev_t dev)
{
    if( dma[dev].pReg != NULL ) {
        ...
        LOGMSG( enError, "Error of dma_s2mm_stop() !!!" );
    }
}
...
{
    dma_reg_t *pReg = dma[dev].pReg;
    return & pReg->mm2s_dmacr;
    return & pReg->mm2s_dmacr;
