...
오후 8:01 2022-11-24	
.Tracking() 모드 값을 추적 해지시 반영하게 해야 함.
.위협 노드 개수 관리 :																	... 완료
	10개일때 방사체-빔 쌍으로 8개까지만 생성 가능
.빔 병합 처리
	...
...
.파일로 PDW 데이터 모의 (진행중) - 나중에 동적시험할 때 필요함.
...
. 지터 펄스열 추출시 누락 펄스열 조건을 엄하게(최대 2개) 해야 할 필요가 있음.
  고정 펄스열 추출과 다르게 해야 함.
. 펄스열 추출시 TOA 만 확인을 했는데 주파수 값도 확인해야 함. -> 기준 펄스열 추출 성공시 주파수 범위를 체크해서 추출.
. 수집 개수가 500개 이상일 때 펄스열 추출 시간이 초 단위로 증가됨.

. 신호 수집한 펄스열은 Normalize 해서 분석을 하게 한다. 이렇게 하지 않은면 펄스열 간의 비교시 TOA 로 비교할 수 있는데...
. 불규칙성 PRI 하모닉 비교 공통으로 구현
