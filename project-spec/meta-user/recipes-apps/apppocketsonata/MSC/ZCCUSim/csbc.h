 ...
 *
 */
class CSBC : public CThread
{
private:
    static CSBC *m_pInstance[2];											///< 싱글톤 객체 입니다.
...
private:
    ...
    void ResultOfOPStart();
    void ResultOfOPShutdown();
    void ResultOfOPRestart();
    void ResultOfUpdateThreatData();
    void ResultOfLostBeamData();
    void ResultOfDeleteThreatData( unsigned int uiDataLength );
    void ResultOfUserDeleteThreatData();
    void ResultOfDeleteBeamData();
    void ResultOfReloadLibrary();
    void ResultOfSetSys();
    void ResultOfSys();
    void ResultOfScanData();
    void ResultOfUserScan();
    void ResultOfDelete();
    void ResultOfSysError();

public:
    CSBC( int iKeyId, char *pClassName=NULL, bool bArrayLanData=false, void *pParent=NULL );	///< 초기 멤버 변수값등을 설정하는 객체 생성자 입니다.
    ~CSBC();																			///< 종료 메시지와 메머리 해지를 처리하는 객체 소먈자 입니다.
