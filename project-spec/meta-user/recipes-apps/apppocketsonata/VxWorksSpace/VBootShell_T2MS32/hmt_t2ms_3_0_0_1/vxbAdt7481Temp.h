...
#ifndef HMT_T2VPX_3_0_0_0_VXBADT7481TEMP_H_
#define HMT_T2VPX_3_0_0_0_VXBADT7481TEMP_H_

#include <hwif/vxBus.h>

#define BIT(x)		( (unsigned int) 1 << x)

#define BSP_ADT7481_SLAVE_ADDRESS				0x4C
#define BSP_TEMP_INT_STACK_SIZE					10240
#define I2C_BUS_NUM								0x00

#define ADT7481_I2C_ADDR_FLAGS					0x4C
...
typedef struct vxbAdt7481Temp {
	VXB_DEV_ID		pAdt7841Temp;
	SEM_ID			adt7481TempAccessSem;
	SEM_ID			adt7481TempIsrDaemonSem;
	UINT32			tempI2cBusNum;
	UINT32			tempSlaveAdrs;
	UINT8			overTempVal;
	UINT32 			overTempDisp;
	UINT8			Hysteresis;
	VXB_RESOURCE *	intRes;
} VXB_ADT7481_TEMP_DRV;
...
int temp_alert_limit(int channel, UINT8 limit_c, UINT8 hysteresis);
/*
 ...
 */
int temp_therm_limit(int channel, UINT8 limit_c, UINT8 hysteresis);

STATUS tempShow(void);
UINT16 tempInfo(void);
UINT8 tempGet(const int offset);
STATUS tempSet(const int offset, UINT8 data);

int GetBoardTemp();

void GetTemperature( UINT32 *puiBoardTemp, UINT32 *puiCPUTemp );

#ifdef __cplusplus
}
#endif

#endif /* HMT_T2VPX_3_0_0_0_VXBADT7481TEMP_H_ */
