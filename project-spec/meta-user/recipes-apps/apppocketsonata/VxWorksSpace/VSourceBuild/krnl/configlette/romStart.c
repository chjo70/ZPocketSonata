...
/*
 * Copyright (c) 1997-1998, 2001-2005, 2007-2014, 2017 Wind River Systems, Inc.
 *
...
--------------------
23Jan17,ghl  support for OpenAMP (F8373)
31oct14,xms  Add BSP_BOOT_CACHE_SYNC. (VXW6-83631)
03apr14,my_  cast BOOT_LINE_ADRS when used (VXW7-2770)
25feb14,ten  Removed legacy header file usrConfig.h
19feb14,rbc  VXW7-1583 - Remove INCLUDE_UEFI_BOOT_SUPPORT
11oct13,my_  change inflate() to vxInflate()
04jan12,c_l  Remove the build warning for default_romResident image.
             (WIND00317509)
11aug11,z_l  Add BSP_BOOT_CACHE_SYNC_POST. (WIND00265986)
04aug11,d_c  Defect WIND00291057 Fix WIND00291556.
             Add long_call attribute to all functions when compiling
	         with GNU and INCLUDE_FAST_LOAD is enabled.
24mar11,j_l  LP64 non-compressed non-uefi bootApp adaptation for I80X86 only.
17jan11,pgh  Add MIPS LMM support.
13jan11,pgh  Speed up ROM image boot time.
07jul10,jb   Fix for Defect WIND00222124 - Bootline not initialized on cold
             boot
18may10,cwl  add the summary of macro "RAM_DST_ADRS". (WIND00198042)
10feb10,j_l  LP64 compressed non-uefi bootApp adaptation for I80X86 only.
18sep09,slk  make sure jump to absEntry is in unampped space for MIPS
21oct08,pch  CQ135132: generate a build error if bad mappings cause
...
16jul07,dcc  Added SDA initialization for MIPS, although not yet enabled.
18jun07,to   fixed data segment copy size for ROM_RESIDENT
08may07,dcc  added SDA initialization for PPC in romStart().
07apr05,cjj  fixed copyLongs() compiler warnings (SPR107455)
16sep04,j_b  correct RAM clearing (SPR 99727)
19aug04,j_b  clear BOOT_LINE_ADRS on cold boot (SPR 100637)
18aug04,j_b  PM_RESERVED_MEM is dependent on INCLUDE_EDR_PM
11may04,j_b  add support for PM_RESERVED_MEM
13jan03,j_b  add ARM THUMB support for ROM_COPY and ROM_COMPRESSED images
             (SPR 80008)
26mar02,sbs  correcting data section relocation for ROM compressed images
             (SPR 73785)
29nov01,sbs  Adding dual copy mechanism for copying data sections from
             ROM to RAM using new WRS labels for sdata and edata.
27nov01,tpw  Manually unroll copyLongs and fillLongs.
06jul98,jmp  added USER_RESERVED_MEM definition if USER_RESERVED_MEM is
             not defined.
20may98,ms   minor cleanup to docs and code.
22aug97,ms   derived from 04p of bootInit.c and simplified.
*/
...
void romStart
     ...
     * section in RAM.  The copyLongs() function and the vxInflate() functions
...
#if !defined(INCLUDE_OPENAMP_ELFLOAD_REMOTE)
    /* OpenAMP's elf-loader will load data segment into RAM */
	
    copyLongs ((UINT *)ROM_DATA_ADRS, (UINT *)UNCACHED(RAM_DATA_ADRS),
        ((UINT)(void *)wrs_kernel_data_end - (UINT)RAM_DATA_ADRS) / sizeof (long));
#endif  /* !defined(INCLUDE_OPENAMP_ELFLOAD_REMOTE) */
