...
/*
 * Copyright (c) 2001-2008, 2013-2014 Wind River Systems, Inc.
 *
...
23jul13,xhs  Support iplite command.
07jul08,pgh  Add support for SATA.
15oct07,uol  Adjustment of fix for defect WIND00104919, limit interface
                 names to IFNAMSIZ to adhere to ipnet_cmd_ifconfig().
09oct07,uol  Fixed defect WIND00104919, allow 64 byte device name in
                 usrNetBootConfig(), and return error if longer.
27aug07,tkf  Add IPv6Only configuration support.
28apr07,tkf  Merge from NOR-SMP sandbox.
02feb07,tkf  Use ipcom_mcmd_ifconfig() and ipcom_mcmd_route() instead of
                 ipnet_cmd_ifconfig() and ipnet_cmd_route().
29jan07,jmt  Add support for USB disks
02feb07,tkf  Use function pointers to access ifconfig and route commands.
05oct06,kch  Replaced do_autoconfig() and usrNetIpv6Attach() with
                 usrNetBootAutoConfig() and addGateway() with
                 usrNetBootAddGateway(). Updated usrNetBootConfig() to work
                 with IPNet.
24feb05,spm  removed unneeded routeFormat values and macros (SPR #100995)
24feb05,spm  performance updates and code cleanup (SPR #100995)
30sep04,dlk  Removed obsolete declaration of random().
28sep04,niq  Zero out the sockaddr structures in the addGateway routine.
17sep04,niq  scale out routing sockets
04aug04,vvv  check inet_addr() return value (SPR #94763)
19may04,snd  Fixed SPR#89215,DHCP client unable to obtain lease when
                 booting from non-network device 
07may04,vvv  fixed warnings (SPR #96364)
10feb04,elp  usrNetBootConfig() returns without message when devName==NULL.
06jan04,rp   fixed SPR 92684
16oct02,ham  fixed improper default gateway configuration (SPR 83122).
11oct02,ham  fixed improper netIoctl return value check.
04oct02,ham  fixed improper ifioctl return value check (SPR 82778).
18sep02,nee  removing ifr_type for SIOCSIFFLAGS
22jul02,rvr  dynamic memory allocation/free for rtmbuf (teamf1)
15jul02,nee  SIOCSIFADDR replaced by SIOCSIFADDR+SIOCSIFFLGS
27feb02,ham  wrote addGateway to add default gateway by routing socket.
06feb02,ham  renamed to usrNetBootConfig due to ifconfig.
05dec01,ham  consolidated usrNetConfigIf.c.
*/
