...
--------------------
13may16,rjq  Enable IPCOM_SPINLOCK_USE_MUTEX by default.(V7NET-831)
07sep15,wzc  INCLUDE_WVNETD should not be defined.(V7COR-3476)
03apr15,h_k  added IPCOM_USE_VXWORKS_LOG_DAEMON config. (F2865)
*/
...
/*
 *===========================================================================
 *                 IPCOM_USE_NATIVE_LINE_EDIT
 *===========================================================================
 * Define this if you want to use the native tyLib line editing in ipcom
 * commands such as the FTP client that accept user input, as opposed to
 * line editing provided by ipcom_getline().  Enabling this will effectively
 * disable the history and editing mechanism provided by ipcom_getline().
 */
#undef IPCOM_USE_NATIVE_LINE_EDIT
 ...
 *===========================================================================
 * Define to implement ipcom_spinlocks as mutex semaphores with 
 * protection enabled against unbounded priority inversion. Undefine
 * this macro to use a generally faster atomic test-and-set strategy. 
 * However, this alternate strategy is susceptible to unbounded priority
 * inversion, and therefore cannot be used in systems requiring 
...
/*
 *===========================================================================
 *                    IPCOM_USE_VXWORKS_LOG_DAEMON
 *===========================================================================
 * Define to use VxWorks syslogLib and tLogTask as a logging daemon to support
 * ipnet_syslog feature.
 */
#define IPCOM_USE_VXWORKS_LOG_DAEMON


/*
 *===========================================================================
 *                    IPCOM_HAS_STACK_INDEX_FUNC
 *===========================================================================
 * Defined when the port provides an ipcom_stack_index() function (which
 * should be more efficient than the default ipnet_stack_index() 
 * implementation).
 */
#define IPCOM_HAS_STACK_INDEX_FUNC

/*
 *===========================================================================
 *                    IPCOM_HAS_STACK_SUSPEND_RESUME
 *===========================================================================
 * Defined when the port provides functions 
 *
 * void *  ipcom_stack_suspend(void);
 * void    ipcom_stack_resume(void *handle);
 * Ip_bool ipcom_stack_exclusive(void);
 *
 * callable from either an application task or from the primary network stack
 * instance.
 *
 * ipcom_stack_supend() must "suspend" all network stack contexts other than
 * the caller (keeping them out of any critical code).  The return value from
 * ipcom_stack_suspend() is passed to ipcom_stack_resume() to resume all
 * network stack instances suspended by ipcom_stack_suspend().
 * ipcom_stack_resume shoud be called from the same task that called
 * ipcom_stack_suspend().
 * ipcom_stack_exclusive returns IP_TRUE if the calling network task has
 * suspended the stack and has exclusive access to global stack data.
 *
 * A NULL return from ipcom_stack_suspend() indicates failure to suspend
 * (e.g. due to a memory allocation failure or a competing suspend attempt).
 */
#define IPCOM_HAS_STACK_SUSPEND_RESUME

/*
 *===========================================================================
 *                    IPCOM_RWSEM_VX_SEM_RWMIN
 *===========================================================================
 * Define when ipcom reader-writer semaphores should be implemented using
 * semRWMinLib semaphores. These are faster than the standard VxWorks RW
 * semaphores (semRWLib), but do not support priority inversion safety
 * or deletion safety.
 */
#define IPCOM_RWSEM_VX_SEM_RWMIN

/*
 *===========================================================================
 *                    IPCOM_RWSEM_MODE_SPLIT
 *===========================================================================
 * Define when the read-specific and write-specific RW semaphore APIs
 * ipcom_rwsem_wait_rd(), ipcom_rwsem_wait_wr(), ipcom_rwsem_post_rd(), and
 * ipcom_rwsem_post_wr() are fundamental, and the non-specific APIs
 * ipcom_rwsem_wait() and ipcom_rwsem_post() are defined as inline functions
 * in terms of the specific functions.
 * When undefined, it is the other way around.
 */

#ifdef IPCOM_RWSEM_VX_SEM_RWMIN
#define IPCOM_RWSEM_MODE_SPLIT
#endif
