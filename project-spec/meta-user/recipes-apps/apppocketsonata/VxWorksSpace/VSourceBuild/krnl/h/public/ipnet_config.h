...
-----------------------
08nov16,rjq  enable SIOCSNDASYNC by default for a non-blocking socket. (V7NET-1142)
28jun16,rjq  Set the net daemons in runtime in SMP mode.  (V7NET-811)
15may16,h_x  Port TSR defect fix forward from vxworks6.9 into vx7. (F6988)
28jan16,wfl  Define IPNET_USE_SO_CONNTIMEO for (V7NET-682)
20jul14,chm  Qos advanced components.(US36943)
...
struct Ipnet_pkt_pool_slab_desc
{
    ...
    Ip_u32         flags;       /* The IPCOM_SLAB_F_xxx flags for
                                   this buffer */
    Ip_u32         empty_lim;   /* Allow slab cache to hold up to
				   this number of empty slabs without
				   triggering immediate garbage
                                   collection */
 ...
 * However, if IPIPSEC2 is defined, senders try to allocate additional
 * packet buffer space for IPsec headers, regardless of whether such headers
 * will actually be present.  To allow maximal-sized sends to succeed
 * in such a build (when the IPsec headers are NOT actually present), we
 * increase the maximum allocatable packet buffer size if IPIPSEC2 is defined.
 * 64 should actually be IPIPSEC_IPSEC_HDR_MAX_SIZE.
 */
#ifdef IPIPSEC2
#define IPNET_PKT_POOL_MAX_MTU (65535 + 40 + 64)
#else
 ...
 *===========================================================================
 * Defines maximum amount of destination cache number the stack is allowed to use.
 * Set to 0 if do not limit(default), but it will still be limited by memory size. 
 */
#ifndef IPNET_DST_CACHE_LIMIT
 ...
 *===========================================================================
 * Makes IPNET able to run old socket code expecting a 16-bit sa_family
 * (or sin_family) member in struct sockaddr (or sockaddr_in) instead of the
 * newer 8-bit sa_len (sin_len) and 8-bit sa_family (sin_family) used when
 * IPCOM_USE_SA_LEN is defined.
 *
 * Specifically, this option supports code that is simply ignorant of the
 * sa_len (or sin_len) member but is built against the current header files
 * that include that member (assuming IPCOM_USE_SA_LEN is defined) and the
 * 8-bit family member.  This option also should suffice for code built againt
 * older headers that specify the 16-bit sa_family member and no sa_len, if
 * built for a big-endian architecture.
 */
#define IPNET_COMPAT_OLDSOCK

/*
 *===========================================================================
 *                         IPNET_BINCOMPAT_OLDSOCK
 *===========================================================================
 * Makes IPNET able to run old socket code expecting a 16-bit sa_family
 * (or sin_family) member in struct sockaddr (or sockaddr_in) instead of the
 * newer 8-bit sa_len (sin_len) and 8-bit sa_family (sin_family) used when
 * IPCOM_USE_SA_LEN is defined.
 *
 * Specifically, this option attempts a fix-up for little-endian binary
 * applications built against headers that specify a 16-bit sa_family
 * (or sin_family) member and no sa_len (or sin_len). (For big-endian
 * applications, only IPNET_COMPAT_OLDSOCK would be needed.)
 *
 * Note, however, that this fix-up can interfere with the interpretation
 * of AF_UNSPEC socket addresses, such as might be passed to connect()
 * to disconnect a UDP socket.
 */
#if defined(IPNET_COMPAT_OLDSOCK) && defined(IP_LITTLE_ENDIAN) && 0
#define IPNET_BINCOMPAT_OLDSOCK
#endif
 ...
 */
#if !defined(IPCOM_FORWARDER_NAE) && !defined(IP_PORT_LXI)
...
/*
 *===========================================================================
 *                         IPNET_USE_SOCKET_PKTAVAIL_CB
 *===========================================================================
 * Define to enable IP_SOEVENT_CB_PKTAVAIL event callbacks to a socket
 * event callback function (event_cb member in Ipcom_socket).  In VxWorks,
 * the event_cb member is reserved to support select() on sockets, and
 * the select() implementation does not care about this event.
 */
#ifndef IP_PORT_VXWORKS
#define IPNET_USE_SOCKET_PKTAVAIL_CB
#endif

/*
 *===========================================================================
 *                         IPNET_SOCK_SENDMSG_ASYNCHRONOUS_IN_NONBLOCK_MODE
 *===========================================================================
 * Define to enable asynchronous sends for nonblocking sockets. In 
 * asynchronous sends, the sending task does not wait for confirmation
 * from the stack that a route to the send's destination could actually
 * be found.
 */
#define IPNET_SOCK_SENDMSG_ASYNCHRONOUS_IN_NONBLOCK_MODE
...
/*
 *===========================================================================
 *                 IPNET_USE_PACKET_CACHE_PER_INSTANCE
 *===========================================================================
 * Enable per-network-stack-instance packet caches.
 * If defined, IPNET will enable per-stack-instance packet caches
 * that may be used only by code known to be running in a network stack task.
 * These caches may be used in preference to the global ipnet packet pool 
 * and its global cache for certain performance-critical allocations.
 * Only bare Ipcom_pkt headers and 'ethernet-mtu' sized packets are kept
 * in the cache.
 * Enabling this feature may increase the packet buffer usage over
 * the short term.
 */
#if defined (IPNET_USE_PACKET_POOL) && defined(IP_PORT_VXWORKS)
#define IPNET_USE_PACKET_CACHE_PER_INSTANCE
#endif
 ...
 ****************************************************************************
 */
#if defined(IPNET_MSP_DISTRIBUTION) && defined(IP_PORT_VXWORKS) && defined(_WRS_CONFIG_IPNET_QOS) && (defined(_WRS_CONFIG_IPNET_DIFFSERV))
#define IPNET_USE_DIFFSERV
#endif

#if defined(IPNET_MSP_DISTRIBUTION) && defined(IP_PORT_VXWORKS) && defined(_WRS_CONFIG_IPNET_QOS)
#define IPNET_USE_CLASSIFIER
#endif

#if defined(IPNET_MSP_DISTRIBUTION) && defined(IP_PORT_VXWORKS) && defined(_WRS_CONFIG_IPNET_QOS)
#define IPNET_USE_IFQUEUE
#endif

#if defined(IPNET_MSP_DISTRIBUTION) && defined(IP_PORT_VXWORKS) && defined(_WRS_CONFIG_IPNET_QOS)
#define IPNET_USE_QOS
#endif
 ...
 *
 */
#if    defined(IPNET_USE_BOND)          \
    || defined(IPWRNAD)                 \
    || defined(IPCOM_FORWARDER_NAE)     \
    || defined(IPMACSEC)                \
    || defined(IPNET_USE_VRRP)          \
    || defined(IPDHCPC6)                \
    || defined(IP_PORT_LXI)
...
*===========================================================================
* Define to solve the issue that in RTP mode network connect by connectWithTimeout would fail.
*/
...
/*
 *===========================================================================
 *                       ipnet_conf_max_instances
 *===========================================================================
 * Maximum number of stack instances that might run at the same time.
 * All the stack instances share the same state (same FIB entries,
 * same set of network interface, same addresses). All instances share
 * things like the socket namespace. Running more than one instance of
 * the stack in the same system will increase the potential
 * scalability across multiple CPU cores. But even an uni-core system
 * may use multiple instances as each stack is backed by its own stack
 * task, which is scheduled independently by the OS. So different
 * stack instances might run at different priority to minimize latency
 * for any task using a high priority stack.
 */
IP_EXTERN int ipnet_conf_max_instances;
...
/*
 *===========================================================================
 *                    ipnet_conf_intercom_congestion_limit
 *===========================================================================
 * Fullness level at which an inpet intercom signal queue is considered
 * congested.  When the queue is congested, some received packets (TCP initial
 * SYNs) are dropped rather than relayed.
 */
IP_EXTERN IP_CONST unsigned ipnet_conf_intercom_congestion_limit;

/*
 *===========================================================================
 *                         ipnet_dst_cache_number_limit
 *===========================================================================
 * Defines maximum amount of destination cache number the stack is allowed to use.
 * Set to 0 if do not limit(default)
 */
IP_EXTERN IP_CONST unsigned ipnet_dst_cache_number_limit;
