...
/*
DESCRIPTION
This library contains the API for IPCOM Hash Tables.

Note, any mutual exclusion for such tables must be provided externally;
the library functions themselves provide none.

Some library routines may allocate and free memory, by default using
ipcom_malloc(), ipcom_calloc(), and ipcom_free().
*/
...
*
* Warning: A "snapshot" of all of the object pointers in the hash table is taken
* before calling the cb_func() function for any object.  Thus, the list of object
* pointers for which ipcom_hash_for_each() calls cb_func() is not affected if one
* of the calls to cb_func() adds or removes objects from the hash table.
*
* Parameters:
* \is
* \i <head>
