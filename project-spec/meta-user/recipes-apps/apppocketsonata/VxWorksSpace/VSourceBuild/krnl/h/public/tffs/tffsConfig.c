...
/*
*  Copyright (c) 1984-2001, 2002, 2004, 2006, 2009, 2016 Wind River Systems, Inc.
*
...
--------------------
21nov16,ffu  add LP64 support in TFFS_DRV layer (F4496)
05feb16,dee  fix compiler warning
06jul09,???  Updating version number for VxWorks 6.8
12jun08,kkz  Updating version number for 6.7 release
23mar06,jb3  Bump Version for Spring 06.
23nov04,mil  Fixed compilation warning. (SPR 103341)
02oct02,nrv  fixing diab build error
28mar02,nrv  adding INCLUDE_MTD_CFIAMD
12dec01,nrv  supressing the compiler warning on tffsVersion
07nov01,yp   fixing SPR #63838
02nov01,nrv  changing the version number to 2.2
17sep01,yp   merging code changes from T3. Preserving T2 specifics that
             were removed in T3
25apr01,mem  Remove NFTL, CDSN and DOC2.
20dec00,tlc  Add user-defined "Identify" routine to MTD table.
23apr01,yp   Incrementing version number to 2.1
29nov00,tlc  Added MTD_USR_IDENTIFY to the mtdTable. This allows
             user-defined identify routines to be used for trueFFS
             support.
06Sep00,csh  Added code to free pBuf on error SPR #34246
06Sep00,csh  Added check for pBuf == NULL SPR #34246
01Sep00,csh  Removed MACRO for INCLUDE_SHOW_ROUTINES
04feb99,yp   added MACRO INCLUDE_TFFS_SHOW for better scalability.
21apr98,yp   added tffs subdir to include path
28feb98,kbw  made man page edits to fix problems found by QE
28feb98,kbw  made man page edits
19jan98,hdn  added new function tffsShowAll().
08jan98,hdn  added new MTD for Heurikon Baja4700.
06jan98,hdn  cleaned up tffsShow() and tffsBootImagePut().
18dec97,yp   added documentation on sharing bootrom with TFFS
15dec97,yp   doc cleanup
11dec97,hdn  added tffsRawio()'s return value check in tffsShow().
08dec97,hdn  renamed mkbootTffs() to tffsCopy().
05dec97,hdn  changed tffsSocket[] to LOCAL variable.
05dec97,hdn  added tffsShow() and mkbootTffs().
11nov97,hdn  fixed documentation of flRegisterComponents().
07nov97,hdn  cleaned up.
07jul97,ami  written by Amirban at M-Systems
*/
...
LOCAL char * tffsVersion = RUNTIME_VERSION;	/* WRS version number of TFFS */
...
void tffsShow
    ...
    status = (FLStatus) tffsRawio (driveNo, TFFS_GET_PHYSICAL_INFO, (_Vx_ioctl_arg_t)&info, 0, 0);
    if (status != OK)
	{
...
STATUS tffsBootImagePut
    (
    int driveNo,		/* TFFS drive number */
    ULONG offset,			/* offset in the flash chip/card */
    ...
    if (tffsRawio (driveNo, TFFS_GET_PHYSICAL_INFO, (_Vx_ioctl_arg_t)&info, 0, 0) != OK)
        {
     ...
     */
    if (tffsRawio (driveNo, TFFS_PHYSICAL_READ, (_Vx_ioctl_arg_t)(unitNo * info.unitSize),
	           (_Vx_ioctl_arg_t)(offset % info.unitSize), (_Vx_ioctl_arg_t)pBuf) != OK)
        {
    ...
    if (tffsRawio (driveNo, TFFS_PHYSICAL_WRITE, (_Vx_ioctl_arg_t)(unitNo * info.unitSize),
	           (_Vx_ioctl_arg_t)offset % info.unitSize, (_Vx_ioctl_arg_t)pBuf) != OK)
        {
    ...
    while (unitSize)
	{
        ...
        if (tffsRawio (driveNo, TFFS_PHYSICAL_WRITE, (_Vx_ioctl_arg_t)addr,
            (_Vx_ioctl_arg_t)ix, (_Vx_ioctl_arg_t)pBuf) != OK)
	    {
