...
struct Ipnet_timeout_struct;  /* opaque here; see ipnet_timeout.h */
...
*
* This routine must be called by proxy code with the NAT lock held.
*
...
* The length of time, in seconds, after which the function is called.
* \i <handler>
* A pointer to the function to be called. This function is called with
* NAT mutual exclusion held, and must release NAT mutual exclusion before
* returning by calling ipnet_nat_proxy_unlock().
* \i <cookie>
* A cookie for use by the called function.
* \i <ptmo>
* A pointer that stores the location of a pointer to the timeout structure used
...
/* new-style proxy timeout APIs, for use with embedded Ipnet_timeout structures */

/*******************************************************************************
*
* ipnet_nat_proxy_static_timeout_init - initialize a static timeout
*
* This routine initializes a static timeout so that it may be scheduled
* using ipnet_nat_proxy_static_tmo_reschedule().
*
* Parameter:
* \is
* \i <tmo>
* A pointer to the timeout structure to be initialized, and the
* argument to the <handler> function when it is called.
* \i <handler>
* The function to call when the scheduled timeout expires.
* \i <cookie>
* Optional value to be stored in <cookie> member of <*tmo>.  Since the
* <handler> function is passed <tmo> as an argument, the <cookie> value
...
* ERRNO:
*/
IP_PUBLIC void ipnet_nat_proxy_static_timeout_init
(
...
/*******************************************************************************
*
* ipnet_nat_proxy_static_tmo_reschedule - schedule or reschedule a pre-allocated timeout
*
* The ipnet_nat_proxy_tmo_reschedule() routine schedules (or reschedules, if the
* timeout is already scheduled) an initialized, pre-allocated timeout.   The
* timeout may be initialized with the routine ipnet_nat_proxy_static_timeout_init(),
* which associates a handler function and an optional void pointer with the
* Ipnet_timeout.
*
* Note that the handler function will be called with one argument, which is the
* pointer <tmo>, unless the timeout is cancelled first (which may be done using
* ipnet_nat_proxy_static_tmo_cancel(), NOT ipnet_nat_proxy_timeout_cancel()).  The handler
* function is called with the NAT lock held, and must unlock the NAT lock by 
* calling ipnet_nat_proxy_unlock() before returning.
*
* The timeout <tmo> is not automatically freed when it expires and the handler
* is called.  If desired, the handler function may free <tmo>. Typically,
* however, the timeout structure would be embedded in a larger object.
*
* Parameter:
* \is
* \i <tmo>
* A pointer to the initialized timeout structure.
* \ie
*
* RETURNS: No return value.
*
* ERRNO:
*/
IP_PUBLIC void ipnet_nat_proxy_static_tmo_reschedule
(
    Ip_u32 sec,
    struct Ipnet_timeout_struct *tmo
);
...
/*******************************************************************************
*
* ipnet_nat_proxy_static_tmo_cancel - cancel a pre-allocated timeout
*
* The ipnet_nat_proxy_static_tmo_cancel() routine cancels a pre-allocated timeout.
* The caller keeps ownership of the timeout; this routine does not free it.
*
* The timeout must have been initialized previously, for example using
* the routine ipnet_static_timeout_init(). (See ipnet_timeout.h).
*
* Parameter:
* \is
* \i <tmo>
...
* \ie
*
* RETURNS: IP_TRUE if the timer had been scheduled and was removed from
*          the timer priority queue; IP_FALSE if the timer was not enqueued.
*
* ERRNO:
*/
IP_PUBLIC Ip_bool ipnet_nat_proxy_static_tmo_cancel
(
    struct Ipnet_timeout_struct *tmo
);
...
/*******************************************************************************
*
* ipnet_nat_proxy_unlock - release NAT mutual exclusion
*
* This routine may be called to release NAT mutual exclusion by a timeout
* handler scheduled using ipnet_nat_proxy_static_tmo_reschedule() or
* ipnet_nat_proxy_timeout_schedule().
* Such a handler is called with the NAT lock held and must release
* the NAT lock before returning by calling this routine.
*
* RETURNS: N/A
*
* ERRNO:
*/
IP_INLINE void ipnet_nat_proxy_unlock(void)
{
    IP_EXTERN void ipnet_nat_unlock(void);
    ipnet_nat_unlock();
}
