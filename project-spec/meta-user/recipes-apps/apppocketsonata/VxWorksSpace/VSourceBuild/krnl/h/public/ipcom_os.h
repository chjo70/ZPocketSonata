...
/*
 * Copyright (c) 2006-2013, 2015, 2017 Wind River Systems, Inc.
 *
...
--------------------
01aug17,dlk adding ipcom_task_redoom() (F5593)
*/
...
IP_EXTERN Ip_s16 ipcom_priority_map[IPCOM_PRIORITY_MAX+1];
...
#define IP_CPU_CLR(cpu, cpusetp)    ((cpusetp)->set[(cpu) >> 5] &= (Ip_u32)~(1 << ((cpu) & 31)))
#define IP_CPU_SET(cpu, cpusetp)    ((cpusetp)->set[(cpu) >> 5] |= (Ip_u32)1 << ((cpu) & 31))
...
{
    enum Ipcom_once_state state; /* The current state of this
                                     once-only initialization.*/
    unsigned              cnt;   /* Number of processes waiting for
                                    the initialization function to
                                    finish. */
    Ipcom_sem             sem;   /* Semaphore where processes waiting
                                    for the initialization function to
                                    finish. */
}
Ipcom_once_t;
...
typedef struct Ipcom_proc_struct
{
    ...
    int             stack_idx;   /* stack instance index, or -1 if not a stack
                                    context */
...
#define IPCOM_PROC_FLAG_UNBREAKABLE       (2)       /* Set task unbreakable */
...
/*******************************************************************************
*
* ipcom_enter_critical_region
*
* This routine is only relevant for AMP systems (NAE). It guarantees
* that only one of the AMP systems can enter the critical
* region. There is only one single critical region defined, so this
* function should only be used in non-performance critical parts.
* It is forbidden to call this function recursively.
* This function is defined to be a no-op for non-AMP builds.
*
* RETURNS: N/A
*
* ERRNO: None.
...
IP_PUBLIC void ipcom_enter_critical_region(void);
IP_PUBLIC void ipcom_enter_critical_region2(void);
#else
#define ipcom_enter_critical_region()  IP_NOOP
#define ipcom_enter_critical_region2() IP_NOOP
#endif


/*******************************************************************************
*
* ipcom_exit_critical_region
*
* This routine is only relevant for AMP systems (NAE). Let the
* current context exit the critical region.
*
* RETURNS: N/A
*
* ERRNO: None.
*/
#if defined(IPCOM_FORWARDER_NAE)
IP_PUBLIC void ipcom_exit_critical_region(void);
IP_PUBLIC void ipcom_exit_critical_region2(void);
#else
#define ipcom_exit_critical_region()  IP_NOOP
#define ipcom_exit_critical_region2() IP_NOOP
#endif
...
/*******************************************************************************
*
* ipcom_block_reader_create - create a block object and store its handle for a specific tid
*
* This routine creates a block object, and stores its handle in the <block>
* parameter.
*
...
* Pointer to <block> handle.
* \i <tid>
* The tid to create the block object for
* \ie
*
* RETURNS: 'IPCOM_SUCCESS' or an error value defined in 'include/ipcom_err.h'.
*
...
IP_PUBLIC Ip_err ipcom_block_reader_create
(
    Ipcom_block *block,
    Ip_pid_t    pid
);
#else
#define ipcom_block_reader_create(block,pid) ipcom_block_create(block)
#endif
...
/*******************************************************************************
*
* ipcom_proc_acreate_v1 - create a new IPCOM process
*
* This routine creates a new IPCOM process with the characteristics specified in
* the attribute object <attr>. The process starts execution immediately and
* competes for execution time according to the scheduling rules of the
* underlying operating system, the defrence between ipcom_proc_acreate_v1 and
* ipcom_proc_acreate is that ipcom_proc_acreate_v1 can set the exact priority
...
* ERRNO: \&'IPCOM_ERR_FAILED'
*/
IP_GLOBAL Ip_err ipcom_proc_acreate_v1
(
    const char *name,
#ifdef IP_PORT_LKM
    int (*entry)(void* data),
...
/*******************************************************************************
*
* ipcom_task_redoom - enable deletion attempt flag for calling task
*
* This routine would typically be called after ignoring an EDOOM error
* return, in order to re-activate the deletion attempt flag for the calling
* task so that a subsequent blocking call may again return EDOOM
* (and potentially not ignoring the EDOOM error the second time).
*
* Parameters: N/A
*
* RETURNS: N/A
...
* ERRNO: None.
*/
IP_PUBLIC void ipcom_task_redoom (void);
...
IP_PUBLIC int     ipcom_get_true_cpuid(void);
IP_PUBLIC void    ipcom_mb(void);
IP_PUBLIC void    ipcom_rmb(void);
IP_PUBLIC void    ipcom_wmb(void);

#ifdef IPCOM_HAS_STACK_SUSPEND_RESUME
IP_PUBLIC void *  ipcom_stack_suspend (void);
IP_PUBLIC void    ipcom_stack_resume (void * handle);
IP_PUBLIC Ip_bool ipcom_stack_exclusive(void);
...
/*******************************************************************************
*
* ipcom_stack_index - find current task's network stack index, if any
*
* If the current task is a network stack task, this routine returns
* its stack instance index; otherwise, this routine returns -1.
*
* Parameters:
*
* RETURNS: The stack instance index of the calling network task, or -1.
*
* ERRNO:
*/
IP_PUBLIC int ipcom_stack_index
(
void
);
#endif
