...
--------------------
03mar16,dee redefine time_t to unsigned long to match kernel definitions
12oct15,chm the size of an Ip_fd_mask and that of a _Vx_fd_mask 
            need to be the same.
*/
...
#if defined(IP_RSIZE_T)
 typedef IP_RSIZE_T          Ip_rsize_t;
#else
#ifdef _WRS_CONFIG_LP64
 typedef unsigned long       Ip_rsize_t;
#else
 typedef unsigned int       Ip_rsize_t;
#endif
#endif

#if defined(IP_ERRNO_T)
 typedef IP_ERRNO_T          Ip_errno_t;
#else
 typedef int   Ip_errno_t;
#endif
...
#if defined(IP_PTRDIFF_T)
 typedef IP_PTRDIFF_T        Ip_ptrdiff_t;
#else
 typedef long int            Ip_ptrdiff_t;
#endif

/* process id */
#if defined(IP_PID_T)
 typedef IP_PID_T            Ip_pid_t;
#else
...
/* socket select() type.
   the Ip_fd_mask type needs to match that of _Vx_fd_mask to support
   IPCOM_USE_NATIVE_SOCK_API */
typedef	long         Ip_fd_mask;

/* time type */
typedef unsigned long	     Ip_time_t;
