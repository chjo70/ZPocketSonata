...
IP_EXTERN Ip_bool ipmcp_is_enabled;
...
/*
 * The possible states a entry can have in the ND cache. It is
 * important that none of the state has a value of 0. Transition from
 * 0 to any other state is assume to be detectable by the neighbor
 * code.
*/
typedef enum Ipnet_nd_state_enum
{
    IPNET_ND_UNINIT = 0,    /* Uninitialized entry */
    IPNET_ND_DEAD,          /* This entry cannot be used anymore */
    IPNET_ND_INCOMPLETE,    /* ND request has been sent, but no answer
                               has be received */
    IPNET_ND_REACHABLE,     /* ND entry is valid */
    IPNET_ND_STALE,         /* ND entry is probably valid */
    IPNET_ND_DELAY,         /* Waiting for the peer to ask for our MAC
                               address */
    IPNET_ND_PROBE,         /* ND request has been sent, old IP to MAC
                               mapping exists */
    IPNET_ND_PENDING        /* Waiting for the first packet to be
                             * queued before entering INCOMPLETE
                             * state. */
}
Ipnet_nd_state_t;

/* IP source address selection filter; used by SCTP */

typedef Ip_bool (*Ip_src_addr_filter)(const void *addr, int domain, Ip_u32 zone_id, void *ctx);
...
IP_PUBLIC int ipnet_cmd_ifconfig_common(int argc, char **argv);
IP_PUBLIC int ipnet_cmd_ifconfig(int argc, char **argv);
IP_PUBLIC int ipnet_cmd_sysctl(int argc, char **argv);
IP_PUBLIC int ipnet_sysctl_common(int argc, char **argv);
...
IP_PUBLIC int ipnet_cmd_ip(int argc, char **argv);
...
IP_PUBLIC int ipnet_cmd_vconfig(int argc, char **argv);
#endif

#ifdef IPCOM_SVCONFIG_CMD
IP_PUBLIC int ipnet_cmd_svconfig(int argc, char **argv);
#endif
