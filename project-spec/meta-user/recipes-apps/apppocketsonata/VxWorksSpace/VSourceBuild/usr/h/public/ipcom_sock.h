...
--------------------
19nov15,h_x  Fix V7NET-684
12feb15,jxy  Change IP_FD_SETSIZE default value to 2048 (VXWRAD-99)
...
#define IP_SIOCSSNDMSGAYNC  IP_IOW(SOCK, 15, R, int)
...
#define IP_NFDBITS          (sizeof(Ip_fd_mask) * 8)  /* bits per mask */
...
#define IP_FD_CLR(fd, pfdset)   \
    ((pfdset)->fds_bits[(fd)/IP_NFDBITS] &= ~((Ip_fd_mask)1u << ((fd) % IP_NFDBITS)))
