			...
			if (wp->flags & WEBS_CGI_REQUEST) {
				...
				if (fd == NULL) {
					if( gstat( wp->cgiStdin, &sbuf) != 0 ) {
						fd = gfopen( wp->cgiStdin, "wb" );
					}
					else {
						fd = gfopen(wp->cgiStdin, "ab+" );
					}
				}
 ...
 */
			...
			if (wp->flags & WEBS_CGI_REQUEST) {
				...
				gfwrite( text, gstrlen(text), 1, fd );
				gfwrite( T("\n"), sizeof(char_t), 1, fd );
				//gwrite(fd, text, gstrlen(text));
				//gwrite(fd, T("\n"), sizeof(char_t));
			} else
...
void websDecodeUrl(char_t *decoded, char_t *token, int len)
{
	...
	for (ip = token; *ip && len > 0; ip++, op++) {
		if (*ip == '+') {
			*op = ' ';
		} else if (*ip == '%' && gisxdigit((int)ip[1]) && gisxdigit((int)ip[2])) {
