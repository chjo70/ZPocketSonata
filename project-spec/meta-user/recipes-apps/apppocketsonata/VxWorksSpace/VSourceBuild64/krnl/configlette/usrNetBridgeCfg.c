...
21apr16,l_y  Add bridge aging timeout time.(VXW6-85482)
23mar10,kxr  Do not ipDetach() wireless interface      
26nov07,tlu  Replace INCLUDE_IPCOM_USE_INET6 by INET6
14nov06,tlu  Remove un-used variable addr (defect 69139)
18sep06,tlu  Un-comment ipDetach and ip6Detach
15aug06,tlu  Ported to IPNET stack 
24jun05,svk  Fix another diab compiler warning
27may05,svk  Fix compilation warnings
28apr05,zhu  Set IFF_IFNET6_UP before assigning IPv6 address
19mar05,zhu  Changed the string format to be consistent with NAT and 
             Firewall
28mar05,svk  Allow bridge to attach to IPv6 stack
02feb05,svk  WDB END handling unnecessary if WDB_END_DEVICE_NAME and
             WDB_END_DEVICE_UNIT are defined
19oct04,svk  Include config.h for command-line BSP build
07oct04,svk  Detach bridge ports from IPv6 stack also
13sep04,svk  Fix compilation warnings
06jul04,zhu  Updated for dual stack
02jul03,myz  Added INCLUDE_WDB_COMM_END macro to the WDB END check 
03jun03,myz  Added WDB END handling when its interface is a bridge port
30may03,myz  Modify promiscuousModeSet to use driver's ioctl to set the mode
15may03,svk  Added includes needed for cmdline build, fixed warnings
25apr03,zhu  updated copyright
10may02,svk  Changed mirrorInit() and promiscuousModeSet() to be callable
		         from outside configlette
25apr02,svk  Coding standard and linked list changes
24apr02,vks  code-review changes: linked-list usage, project facility
		         parameter dependency removal (usrNetBridgeInit() signature),
		         several minor changes as per coding standards.
22apr02,svk  Replace ports buffer on stack with malloc'ed buffer
11apr02,svk  Add function to use socket ioctl.
10apr02,svk  More minor changes.
10apr02,vks  merged bridge and mirror configlettes
10apr02,svk  Various minor changes.
09apr02,svk  written
*/
...
STATUS usrNetBridgeInit
    ...
    int     timeout      /* bridge aging timeout time */
    )
    {
    ...
    /* RFC 6325: 4.8.3. Forgetting End-Station Addresses 
     ...
     */
    if (timeout > 1000000 || timeout < 10)
    {
        BridgeAgingTime = 300;
    }
    else
    {
        BridgeAgingTime = timeout;
    }
...
number pairs.\nFor example: \"fei,0,fei,1,elPci,0\"\n");
    ...
    *ptr = '\0';
    ...
    for (token = strtok_r(pPortsBuf, PORT_SEPARATOR, &pHolder); token != NULL;
        ...
        if (endFindByName(devName,unit) == NULL)
            {
	...
	/* Add the node to the list */
        ...
        if (pNode == NULL)
            {
            printf("Can't allocate memory for device %s%d\n", devName, unit);
            status = ERROR;
            break;
            } 

        bzero((char*) pNode, sizeof(BRIDGE_NODE));
        strcpy(pNode->devName, devName);
        pNode->unit = unit;
...
LOCAL STATUS bridgePortsInit 
	...
	if (oldifname != IP_NULL)
    	{
#if defined(_WRS_CONFIG_COMPONENT_IP8021X)   \
    && defined( _WRS_CONFIG_COMPONENT_DOT1X) \
    && defined( _WRS_CONFIG_COMPONENT_IPWPS)
           /* Consider the following scenario:
            *   - a wireless interface is being added to the bridge;
            *   - the ip8021x supplicant has opened a socket on the wireless interface.   
            * In this scenario, ipDetach() should not be called.
            * Scenario occurs only when have the ip8021x supplicant, the WPS module 
            * and dot1x authenticator.
            */
           if (*pNode->devName != 'w')
#endif
           {
     ...
     !defined(WDB_END_DEVICE_NAME) && !defined(WDB_END_DEVICE_UNIT))
        ...
        netJobAdd ((FUNCPTR)devicePromiscuousModeSet,(_Vx_usr_arg_t)pEnd,0,0,0,0);
...
/*******************************************************************************
*
* configMirrorInterface - IPv4 mirror interface configuration
*
* This routine attaches the IPv4 stack to the mirror device.  It also sets
* the IP address and mask
*
*
* RETURNS:
* OK, or
* ERROR if there is a problem in configuring the mirror interface
*
* NOMANUAL
*/

LOCAL STATUS configMirrorInterface
    (
    char*   mirrorDevName,
    ...
    ip_if_name = (char *)ipcom_get_ip_if_name(mirrorDevName);
    if(ip_if_name == IP_NULL)
        {
        printf("ipAttach failed to register dev name mapping for %s\n",
            mirrorDevName);
        goto bailout;
        }
    strcpy(ifr.ifr_name, ip_if_name);
...
/*******************************************************************************
*
* configMirrorInterface6 -  IPv6 mirror interface configuration
*
* This routine attaches the IPv6 stack to the mirror device.  It also sets
* the IP address.
*
* RETURNS:
* OK, or
...
*/
#ifdef INET6
LOCAL STATUS configMirrorInterface6
    (
    char*   mirrorDevName,
    ...
    ip_if_name = (char *)ipcom_get_ip_if_name(mirrorDevName);
    if(ip_if_name == IP_NULL) {
      printf("ip6Attach failed to register dev name mapping for %s\n",
          mirrorDevName);
        goto bailout;
    }
    strcpy(ifareq6.ifra_name, ip_if_name);
...
STATUS mirrorInit
    ...
    if (gotAddrv4)
        {
#endif
        if(configMirrorInterface(mirrorDevName, pAddr, mask) != OK)
            {
            muxDevUnload (MIRROR_DEV_NAME, MIRROR_STACK_UNIT_NUM);
            return ERROR;
            }
    ...
    if (gotAddrv6)
        {
        if(configMirrorInterface6(mirrorDevName, pAddrv6, prefixLen) != OK)
            {
            muxDevUnload (MIRROR_DEV_NAME, MIRROR_STACK_UNIT_NUM);
            return ERROR;
            }
...
LOCAL void devicePromiscuousModeSet
    ...
    if (pEnd->pFuncTable->ioctl != NULL)
      pEnd->pFuncTable->ioctl (pEnd, (int)EIOCSFLAGS, (void *)(long)ifFlags);
    }
