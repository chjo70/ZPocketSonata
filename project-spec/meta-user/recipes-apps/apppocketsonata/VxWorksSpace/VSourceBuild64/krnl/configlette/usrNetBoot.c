...
/*
 * Copyright (c) 2002-2008, 2012, 2014, 2016 Wind River Systems, Inc.
 *
...
04dec14,chm  Compiler warning from usrNetBoot.c when building PPC VIP.(V7NET-440)
04nov14,jpb  added header file for _func_printErr (V7COR-910)
27oct14,lzq  Fix warning V7NET-369
26feb14,jsh  Include funcBindP.h
16jan12,mry  Fix for WIND00328320 (Share memory network name is incorrect if the boot loader parameter is defined).
20nov08,pgh  Fix the parsing of the other bootline parameter.
13aug08,dtr  Fix DevNameGet to use unit number.WIND00130307
07jul08,pgh  Add support for SATA.
25aug07,tkf  Add usrNetBootIpv6AddrExtract() as part of a fix for
             CQ:WIND00102553).
29jan07,jmt  Add USB support to usrNetDevNameGet
06oct06,kch  Added netInit boolean.
19sep06,mze  added check in usrNetBoot for INCLUDE_SM_NET for WIND00058841
12sep05,vvv  fixed build error (SPR #105297)
10feb04,elp  print error message when no device is found (replaces
...
17jun03,vvv  removed call to usrNetmaskGet() (SPR #88786)
15may03,spm  updated to Tornado 2.2 FCS release (added SPR #73762 fix)
07may03,syy  Fixed compiler warnings
06mar02,ham  moved usrNetmaskGet() from usrNetBootUtil.c
05mar02,ham  fixed compilation warnings.
28feb02,ham  call usrNetmaskGet in usrNetBoot.
28feb02,ham  call usrNetDevNameGet in usrNetBoot.
26feb02,ham  ported TOR3_1-FCS (01g).
*/
...
LOCAL char bootDev[BOOT_DEV_LEN];
...
void usrNetmaskGet (void)
    ...
    if (!netDevBootFlag)
        {
        netmask = 0;
        (void)bootNetmaskExtract (pAddrString, &netmask);
        }
    }
...
void usrNetDevNameGet
    ...
    for (i = 0; i < BOOT_DEV_LEN; i++)
        {
        if ((isalpha ((int)sysBootParams.bootDev[i]))||(sysBootParams.bootDev[i]=='_'))
            {
    ...
    *pNetUnit = sysBootParams.unitNum;
    ...
    if ( (strncmp (bootDev, "scsi", 4) == 0) ||
        (strncmp (bootDev, "fs", 2) == 0) ||
        (strncmp (bootDev, "ide", 3) == 0) ||
        (strncmp (bootDev, "ata", 3) == 0) ||
        (strncmp (bootDev, "fd", 2) == 0)  ||
        (strncmp (bootDev, "tffs", 4) == 0) ||
    ...
    else if (strncmp (bootDev, "passDev", 7) == 0)
    	{
	*pNetDev = NULL;
	}
    return;
