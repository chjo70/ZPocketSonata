...
/*
 * Copyright (c) 1992-2009, 2017 Wind River Systems, Inc.
 *
...
--------------------
02jun17,aeg  added loading of signed symbol table file (F7459)
03aug09,pgh  Fix boot device parsing.
21apr09,pgh  Fix the parsing of boot devices.
11jul08,spw  Remove DHCP delay
07jul08,pgh  Add support for SATA.
06jun08,spw  Wait for successful DHCP client negotiation to complete before
             attempting to load symbol file.
30apr08,v_r  Network symbol table special flags handling now done in
             loader code.
03mar07,jmt  Defect 89654- Symbol file does not load when booting from USB
             drive
24feb07,jmt  Remove calls to Config routines.  This is done in
             usrConfig.c.
30jan07,jmt  Add support for USB disks
10jan06,bpn  Moved system symbol table initialisation to usrSysSymTbl.c
02mar05,v_r  Included loadLibP.h
02feb05,elp  fixed SPR #104666 (moved bootDevIsNetwork out of ifdef).
26jan05,v_r  Fixed SPR #30900 (return value of symTblCreate() not checked).
             Updated copyright notice for 2005. Removed PRJ_BUILD define
             around usrLoadSyms() as it is a generic wrapper now.
30aug04,jn   Apply code review changes
02jun04,dtr  Fixed prioritySet placement for SPR 97947.
03jun04,jn   Do not delay task if load is OK
05apr04,jn   Comply with coding conventions
25feb04,jn   Minor code reorganization.  Prepare to put the
             "creation of boot device file system" code into a
             separate component
22jul03,jmp  added support for INCLUDE_PASSFS_SYM_TBL.
24may02,wap  made previous fix conditional on INCLUDE_NET_INIT
10may02,wap  Reduce task priority below tNetTask's before doing netLoad()
             (SPR #76107)
29oct01,jkf  added rip's SPR 62094 & 62122 fixes, made a symlink from
             src/config/usrLoadSym.c to this version.  Changed DESC.
15apr99,sn   fixed SPR 26726 (=SPR 22644):
             prevent netSymTbl from ever firing linked C++ ctors
03jun98,ms   modfied for config tool
19nov97,hdn  added support for TFFS.
18nov96,dbt  ANSIfied function headers (SPR #7427.)
01nov96,hdn  added support for PCMCIA.
23jul96,hdn  added support for ATA driver.
25oct94,hdn  swapped 1st and 2nd parameter of usrFdConfig().
13nov93,hdn  added support for floppy and IDE drive.
18sep92,jcf  written.
*/
...
#ifdef INCLUDE_SECURE_LOADER
#define SYMTBL_FILENAME_FMT   "%s.sym.signed"
#else
#define SYMTBL_FILENAME_FMT   "%s.sym"
#endif /* INCLUDE_SECURE_LOADER */
...
LOCAL STATUS symTblFilenameMake 
    ...
    *pBootDevIsNetwork = FALSE;
    ...
    if (strcmp (bootDev, "scsi") == 0)
       {
       sprintf (pFilenameBuf, SYMTBL_FILENAME_FMT, sysBootParams.bootFile);
       }
#ifdef INCLUDE_IDE
        ...
        *pBootDevIsNetwork = TRUE;
        ...
        sprintf (pFilenameBuf, "%s:" SYMTBL_FILENAME_FMT,
...
STATUS netLoadSymTbl ()
    ...
    if (status == ERROR)
        ...
        /* WRS_XXX jn - only delay in the case of failure, not in the 
