...
/*
 * Copyright (c) 1998,2004-2006,2010,2012,2014,2015 Wind River Systems, Inc.
 *
...
--------------------
15oct15,elp  Fixed printf format for 32 bit variable (V7COR-3701)
29jan14,elp  Removed LOADER dependencies.
27apr12,jmp  CQID: Fix WIND00340092 : Fixed some build warnings.
06aug10,v_r  Added standalone symbol table support for Thumb2 (def
                 CQ:WIND00224982).
10jan06,bpn  Moved system symbol table initialisation to usrSysSymTbl.c
12apr05,v_r  Fixed SPR #107398 (Inclusion of kernel shell components gives
                 a build warning).
31jan05,v_r  Fixed typo from last checkin that caused build failure
26jan05,v_r  Fixed SPR #30900 (return value of SymTblCreate() not
                 checked). Updated copyright notice for 2005.
30sep04,jn   Modify use of config param to avoid build warning in bsp build
24sep04,jn   Add configurability for including absolute symbols in system
                 symbol table or not
23nov98,cdp  added support for ARM_THUMB.
03jun98,ms   written
*/
...
void usrStandaloneInit (void)
    ...
    if (errCnt != 0)
	printf ("%d symbols could not be registered.\n", errCnt);
    }
