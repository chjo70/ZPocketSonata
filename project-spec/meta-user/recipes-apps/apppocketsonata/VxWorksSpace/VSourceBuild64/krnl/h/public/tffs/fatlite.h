/* fatlite.h - True Flash File System */

/* Copyright 1984-2004 Wind River Systems, Inc. */

/*
 * Copyright (c) 1984-2004, 2010-2011, 2016 Wind River Systems, Inc.
 *
...
22apr13,jxu  eTFFS: added FL_GET_FREE_SECTORS for background GC
...
typedef int FLHandle;	/* Handle of an open file or drive.	*/
...
typedef struct {
  ...
  unsigned int  irLength;	/* No. of bytes, size or position for   */
				/* operation			 	*/
} IOreq;
...
typedef enum    {FL_MOUNT_VOLUME,
		 ...
		 FL_PHYSICAL_ERASE,
	         FL_GET_NBLKS,
                 FL_GET_FREE_SECTORS} FLFunctionNo;
...
#define NEED_DEFRAG     0x1     /* indicating defragment is needed */
#define NEED_STATIC_WL  0x2     /* indicating static wear-leveling is needed */
...
#define flGetNBlks(ioreq)    flCall(FL_GET_NBLKS, ioreq)
...
typedef struct {
  ...
  unsigned int		unitSize;		/* Smallest physically erasable size
						   (with interleaving taken in account) */
  unsigned int		mediaSize;		/* media size */
} PhysicalInfo;
