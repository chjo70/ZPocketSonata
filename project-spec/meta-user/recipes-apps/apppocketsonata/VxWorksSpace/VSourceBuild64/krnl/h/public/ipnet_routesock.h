...
-----------------------
08dec16,wfl  Move some functions which is in ifLib.c to cancel VIP dependency (V7NET-1193)
01sep16,h_x  ifAllRoutesDelete asserts and suspends task. V7NET-1077
...
/* Wind river extension for address conflict detection */
#define IPNET_RTM_ADDRCONFLICT 0xe1  /*  F2   Address conflict detected for the address. 
                                      *       IPNET_RTM_DELADDR will follow */
...
#define IPNET_RTM_DELETE_ALL_1 0xa1
#define IPNET_RTM_DELETE_ALL_2 0xa2
...
struct Ipnet_if_data
{
    ...
    Ip_u64  ifi_ibytes;             /* Total number of octets received */
    Ip_u64  ifi_obytes;             /* Total number of octets sent */
 ...
 * TODO:
 *
 * These route-deletion routines need to be adapted to the multiple instance stack.
 * It is likely not safe to use internal ipnet routines to perform routing table
 * walks & such unless running as a network task, but these APIs are really intended
 * to be run by non-network tasks.
 *
 * Need to remove support for them or re-implement them (using routing sockets
 * or some such method).
 */
struct if_rtm{
