...
/*
 * Copyright (c) 2006-2013,2014 Wind River Systems, Inc.
 *
...
#define IP_IFT_L2BRIDGE    0xf8   /* 802.1D Bridge Interfaces */
#define IP_IFT_L2BRIDGE_PV 0xf9   /* 802.1Q Port Based VLAN Bridge Interfaces (a PVID is specified)*/
#define IP_IFT_L2BRIDGE_TV 0xfa   /* 802.1Q Tag Based VLAN Bridge Interfaces */
...
/* proprietary ioctls: enable/disable vlan filtering */
#define IP_SIOCXETHSVLANFILTER    IP_IOWR(ETH, 30, W, struct Ip_ethreq)

/* proprietary ioctls: set/clr VID filtering */
#define IP_SIOCXETHSVIDFILTER     IP_IOWR(ETH, 31, W, struct Ip_ethreq)
#define IP_SIOCXETHCVIDFILTER     IP_IOWR(ETH, 32, W, struct Ip_ethreq)
...
/* Interface supports MACsec SecY */
#define IPCOM_IF_DRV_CAP_MACSEC_SECY     		(1 << 7)
 ...
 *              neigh: pointer to IPNET route entry ( can be used
...
typedef int (*Ipcom_drv_ip_resolve)(void *mux_cookie,void *m_blk,
				    ...
				    int pkt_type, int frame_type, void *neigh);
#endif
...
typedef struct Ipcom_netif_struct
{
    ...
    Ip_u32         softix;             /* 'soft' interface index */
    ...
    Ipcom_mib2_stats_netif *mib2;   /* pointer to array of structures,
                                       one per stack instance */
#endif
...
/*
 *===========================================================================
 *                    ipcom_netif_from_softix
 *===========================================================================
 * Description: Convert a 'soft' (internal-only) network interface index
 *              to the corresponding interface pointer.
 * WARNING:     Soft interface indices can be reused immediately after
 *              an interface is detached.
 * Parameters:  softix - soft interface index
 * Returns:     A pointer to the interface descriptor if found,
 *              IP_NULL otherwise.
 *
 */
IP_PUBLIC Ipcom_netif * ipcom_netif_from_softix(Ip_u32 softix);
