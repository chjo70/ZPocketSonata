...
#define IPCOM_SLAB_F_SYSMEM     (1u << 3) /* Buffers are allocated by
                                             external function */
...
typedef struct Ipcom_memory_cache_struct
{
    ...
    Ipcom_list      slabs_full;   /* List of slabs where all objects
                                     are allocated */
    Ipcom_list      slabs_partial;/* List of slabs that contain both
    ...
    const unsigned  alignment;    /* Minimal alignment of all objects */
    unsigned        empty_lim;    /* Trigger garbage collection if cache
                                     has more than this number of empty
				     slabs (default 1) */
...
/*
 *===========================================================================
 *                       ipcom_slab_free_nogc
 *===========================================================================
 * Description: Returns an object to a slab cache without performing garbage
 *              collection. Safe to call while holding a restricted spinlock.
 *              Allows performing the garbage collection unlocked later
 *              by calling ipcom_slab_gc_empties().
 * Parameters:  mc - slab cache to return the object to
 *              obj - the object to free
 *
 * Returns: IP_NULL if garbage collection is not needed. Otherwise, a
 *          non-IP_NULL value encoding the former contents of the empty
 *          slabs list for the memory cache.
 */
IP_PUBLIC void *
ipcom_slab_free_nogc(Ipcom_memory_cache *mc, void *obj);


/*
 *===========================================================================
 *                      ipcom_slab_extract_empties
 *===========================================================================
 * Description: Retrieve empty slabs from a slab cache.
 * Parameters:  mc - The memory cache from which the empty slabs should
 *                   be extracted.
 * Returns:     Opaque pointer to first empty slab of list of empty slabs
 *              removed from the memory cache, or IP_NULL.
 */
IP_PUBLIC void *
ipcom_slab_extract_empties(Ipcom_memory_cache *mc);


/*
 *===========================================================================
 *                       ipcom_slab_gc_empties
 *===========================================================================
 * Description: Free empty slabs retrieved by ipcom_slab_free_nogc() or
 *              ipcom_slab_extract_empties().
 *              Safe to call unlocked (so long as mc exists).
 * Parameters:  mc - the memory cache from which the empty slabs were extracted
 *              empties - Pointer to first slab of list saved by
 *                        ipcom_slab_free_nogc().
 *                        Do not call this routine if empties is IP_NULL.
 * Returns: N/A
 *
 */
IP_PUBLIC void
ipcom_slab_gc_empties(Ipcom_memory_cache *mc, void *empties);
