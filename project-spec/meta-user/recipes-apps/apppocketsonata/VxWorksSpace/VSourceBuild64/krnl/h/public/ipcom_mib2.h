...
# if defined(IPNET)
#  define IPCOM_MIB2(net, xentry)                  (net)->mib2.xentry
#  ifdef IPCOM_USE_MIB2_NEW
#   define IPCOM_MIB2_U64_INC(net, xentry)         IP_U64_ADD32((net)->mib2.xentry, 1)
#   define IPCOM_MIB2_SYSWI_U32_ADD(net, ver, var, val) (net)->mib2.syswide.ver.var += val
#   define IPCOM_MIB2_SYSWI_U64_ADD(net, ver, var, val) IP_U64_ADD32((net)->mib2.syswide.ver.var, val)
#  else
#   define IPCOM_MIB2_U64_INC(net, xentry)         IP_NOOP
#   define IPCOM_MIB2_SYSWI_U32_ADD(net, ver, var, val) IP_NOOP
#   define IPCOM_MIB2_SYSWI_U64_ADD(net, ver, var, val) IP_NOOP
#  endif
# elif defined(IPLITE)
...
# define IPCOM_MIB2(net, xentry)                        IP_NOOP
# define IPCOM_MIB2_U64_INC(net, xentry)                IP_NOOP
# define IPCOM_MIB2_SYSWI_U32_ADD(net, ver, var, val)   IP_NOOP
# define IPCOM_MIB2_SYSWI_U64_ADD(net, ver, var, val)   IP_NOOP
#endif
 ...
 */
#ifdef IPCOM_USE_MIB2
# define IPCOM_MIB2_NETIF2(xentry, netif, idx) \
    do	\
    {	\
        if (netif != IP_NULL) \
            netif->ipcom.mib2[idx].xentry; \
    } while((0))
# define IPCOM_MIB2_NETIF(xentry, idx) IPCOM_MIB2_NETIF2(xentry, netif, idx)
# ifdef IPCOM_USE_MIB2_NEW
#  define IPCOM_MIB2_PERIF_U32_ADD(ver, var, val, ifc, idx) \
    do  \
    {   \
        if (ifc != IP_NULL) \
            ifc->ipcom.mib2[idx].perif.ver.var += val; \
     } while((0))
#  define IPCOM_MIB2_PERIF_U64_ADD(ver, var, val, ifc, idx) \
    do  \
    {   \
        if (ifc != IP_NULL) \
            IP_U64_ADD32(ifc->ipcom.mib2[idx].perif.ver.var, val); \
    } while((0))
# else  /* #ifdef IPCOM_USE_MIB2_NEW */
#  define IPCOM_MIB2_PERIF_U32_ADD(ver, var, val, ifc, idx) IP_NOOP
#  define IPCOM_MIB2_PERIF_U64_ADD(ver, var, val, ifc, idx) IP_NOOP
# endif
#else /* #ifdef IPCOM_USE_MIB2 */
# define IPCOM_MIB2_NETIF2(xentry, netif, idx)              IP_NOOP
# define IPCOM_MIB2_NETIF(xentry, idx)                      IP_NOOP
# define IPCOM_MIB2_PERIF_U32_ADD(ver, var, val, ifc, idx)  IP_NOOP
# define IPCOM_MIB2_PERIF_U64_ADD(ver, var, val, ifc, idx)  IP_NOOP
#endif
 ...
 *
 */
struct Ipcom_mib2_stats_netif_struct
{
    /* IfTable */
    Ip_u32 ifLastChange;
    Ip_u32 ifInOctets;
...
typedef struct Ipcom_mib2_stats_netif_struct Ipcom_mib2_stats_netif;
