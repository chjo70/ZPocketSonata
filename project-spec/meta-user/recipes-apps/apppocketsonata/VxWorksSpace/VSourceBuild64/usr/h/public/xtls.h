...
#define _XTLS_DTOR_FUNC(name)   _Tls_dtor_func_ ## name
...
#define _XTLS_SETUPX(name)	_Tls_setupx_ ## name
 ...
 #if _MULTI_THREAD && _COMPILER_TLS
  #define _CLEANUP(name)        _Atthreadexit(_XTLS_DTOR(name))
 #elif  defined(__RTP__)
  #define _CLEANUP(name)       __cxa_atexit(_XTLS_DTOR_FUNC(name), &name, &__dso_handle)
 #else /* _MULTI_THREAD && _COMPILER_TLS */
  #define _CLEANUP(name)        _Atexit(_XTLS_DTOR(name))
 #endif /* _MULTI_THREAD && _COMPILER_TLS */
#endif/*_NO_WINDRIVER_MODIFICATIONS*/
...
#else/*_NO_WINDRIVER_MODIFICATIONS*/
        /*
         * The TLS destructor function takes a void pointer as argument, so
         * the function defined via _XTLS_DTOR must too in order to avoid
         * compiler warnings.
         */
   #define _TLS_DEFINE_NO_INIT_DT(scope, type, name, dtor) \
        static _Once_t _XTLS_ONCE(name) = _ONCE_T_INIT; \
        static void(*_XTLS_DTOR_FUNC(name))(void*) = dtor; \
        static void _XTLS_DTOR(name)(void) \
        { \
        _XTLS_DTOR_FUNC(name)(&name);\
        } \
        static void _XTLS_REG(name)(void) \
        { \
        _CLEANUP(name); \
