...
#define IP_IN6_IS_ADDR_UNSPECIFIED(addr) \
    IP_IN6_ARE_ADDR_EQUAL(addr, &ip_in6addr_any)

#define IP_IN6_IS_ADDR_LOOPBACK(addr) \
    IP_IN6_ARE_ADDR_EQUAL(addr, &ip_in6addr_loopback)

#define IP_IN6_IS_ADDR_V4COMPAT(addr) \
    (IP_GET_32ON16((void *)&(addr)->in6.addr32[0]) == 0 && \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[1]) == 0 && \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[2]) == 0 && \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[3]) != 0 && /* Must not return TRUE for "::" and "::1" */ \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[3]) != ip_ntohl(1))

#define IP_IN6_IS_ADDR_V4MAPPED(addr) \
    (IP_GET_32ON16((void *)&(addr)->in6.addr32[0]) == 0 && \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[1]) == 0 && \
     IP_GET_32ON16((void *)&(addr)->in6.addr32[2]) == ip_ntohl(0x0000ffff))
...
#define IP_IN6_IS_ADDR_SITE_LOCAL(addr) \
    (((ip_ntohs((addr)->in6.addr16[0]) & 0xffc0) == 0xfec0) || \
		((ip_ntohs((addr)->in6.addr16[0]) & 0xfe00) == 0xfc00))
...
#define IP_IPV6_X_VRID           103   /* Ip_u8; specify the virtual router ID as ancillary data */
 ...
 *===========================================================================
 */
struct Ip_sockaddr_in6
{
#ifdef IPCOM_USE_SA_LEN
    Ip_u8	        sin6_len;	   /* length of this struct */
#endif
    Ip_sa_family_t  sin6_family;   /* IP_AF_INET6 (8bits) */
    Ip_in_port_t    sin6_port;	   /* Transport layer port, net order */
    Ip_u32	        sin6_flowinfo; /* IP6 flow information, net order */

    struct Ip_in6_addr sin6_addr;  /* IPv6 address, net order */

    Ip_u32	        sin6_scope_id; /* interface scope id, host order */
};
...
typedef struct Ip_pkt_ip6_ext_hdr_struct
{
    ...
    Ip_u8  nxt;
    /*
     * Length of of extension in 8-octets units, not counting the
     * first 8-octets
     */
    Ip_u8  len;
}
...
#else
# define IP_IP6F_OFF_MASK       0xfff8  /* mask out offset from ip6f_offlg */
# define IP_IP6F_RESERVED_MASK  0x0006  /* reserved bits in ip6f_offlg */
